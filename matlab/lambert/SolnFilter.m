function [feas,FeasCodes_index]=SolnFilter(transorbit,filters,constants,mode)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if filters.check_a_bnds==1
    if transorbit.a*constants.RU < filters.a_min || transorbit.a*constants.RU > filters.a_max
        feas(2)=0;
    else
        feas(2)=1;
    end
...
if transorbit.a<transorbit.a_m
    disp('a is less than a_m')
    feas(7)=0;
else
    feas(7)=1;
end
...
[~,~,~,~,~,~,~,tF]=Get_alpha_beta(transorbit.a,transorbit.s,transorbit.c,transorbit.theta,transorbit.N,transorbit.t_des,transorbit.branch,constants);
if abs(tF-transorbit.t_des)>=1e-2
    disp('TF time does not match t_des')
    feas(8)=0;
else
    feas(8)=1;
end
