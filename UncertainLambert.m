...
XX= sphere6Dm(16);
Nmc=size(XX,1);
XX(:,1:3)=XX(:,1:3)*1e-5+repmat(rA(:)',Nmc,1);
XX(:,4:6)=XX(:,4:6)*1e-4+repmat(vA(:)',Nmc,1);
% xs1=xs1*1e-5+rA(1);
% ys1=ys1*1e-5+rA(2);
% zs1=zs1*1e-5+rA(3);
% 
...
hold on
% surf(xs1*constants.normX2trueX,ys1*constants.normX2trueX, zs1*constants.normX2trueX,'FaceColor','green','EdgeColor','none','FaceAlpha',0.4);
plot3(XX(:,4),XX(:,5),XX(:,6),'bo');

% camlight right; lighting phong

figure

tri = delaunay(Xtrajmc1x_tdes(:),Xtrajmc1y_tdes(:));
plot(Xtrajmc1x_tdes(:),Xtrajmc1y_tdes(:),'.')
[r,c] = size(tri);
disp(r)
A=Xtrajmc1x_tdes(:);B=Xtrajmc1y_tdes(:);C=Xtrajmc1z_tdes(:);
h = trisurf(tri, A,B , C ,'FaceColor','green','EdgeColor','none');
...
figure(4)
hold on
plot(Xtrajmc1x_tdes(:)*constants.normX2trueX,Xtrajmc1y_tdes(:)*constants.normX2trueX,'ro')

figure(5)
hold on
plot(Xtrajmc1y_tdes(:)*constants.normX2trueX,Xtrajmc1z_tdes(:)*constants.normX2trueX,'ro')

figure(6)
hold on
plot(Xtrajmc1x_tdes(:)*constants.normX2trueX,Xtrajmc1z_tdes(:)*constants.normX2trueX,'ro')

figure(7)
hold on
plot3(Xtrajmc1x_tdes(:)*constants.normX2trueX,Xtrajmc1y_tdes(:)*constants.normX2trueX,Xtrajmc1z_tdes(:)*constants.normX2trueX,'ro')
